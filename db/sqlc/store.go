...
// Store provides all functions to execute db queries and transactions (Real implementation)
type Store interface {
	Querier
	TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error)
}

// SQLStore provides all functions to execute db queries and transactions (Real implementation)
...
func (store *SQLStore) TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error) {
	...
	err := store.execTx(ctx, func(q *Queries) error {
		...
		// Update account with smaller ID first to prevent deadlock
